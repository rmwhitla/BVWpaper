...
detail<- ggplot(data=world) +
  ...
  geom_point( data=info %>% filter(Original_region == "GB"), 
              size=2, aes(x=longitude, y=latitude), shape=19, alpha=0.75) + 
  geom_point( data=info %>% filter(Original_region == "Europe"), size=2, 
              aes(x=longitude, y=latitude), shape=17, alpha=0.75) + 
  geom_label_repel(data=info %>% filter(Original_region != "Japan"), 
                   box.padding = 0.5, aes(x=longitude, y=latitude, label = short_specimen_name), size = 3)+
  ...
  ggtitle("A") + 
  theme(plot.title = element_text(vjust = -7, hjust=0.02, size = 20, face = "bold"),
...
map <- detail + 
      inset_element(global, left = .7, bottom = .66, top = 1, right = 1.1) + 
...
#Plot the PCA
pca <- angsdPCAdf %>%
  ggplot(aes(x=PC1, y=PC2,shape=Original_region, col= Region_postPCA)) +
  geom_point(size=2, aes())+ scale_shape_manual(values=c(17,19)) +
  scale_color_manual(values=c("dark grey", "#000000"))+
  ...
  ggtitle("B") + 
  theme_bw() + 
  theme(plot.title = element_text(vjust = -7, hjust=.02,size = 20, face = "bold" ),
        panel.grid.major = element_blank(), 
